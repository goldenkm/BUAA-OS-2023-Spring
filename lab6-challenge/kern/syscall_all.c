...
int sys_exofork(void) {
	...
	*/
	...
	strcpy(e->env_cur_path, curenv->env_cur_path);
	e->env_shell_id = curenv->env_shell_id;
	//printk("child_id is %x\n", e->env_id);
	return e->env_id;
}
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if (!(pa >= 0x10000000 && pa + len - 1 < 0x10000020) && !(pa >= 0x13000000 && pa + len - 1 < 0x13004200) && !(pa >= 0x15000000 && pa + len - 1 < 0x15000200)) {
		return -E_INVAL;
	}
...
void sys_get_cur_path(char *path) {
	strcpy(path, curenv->env_cur_path);
}
...
int sys_unset_env_var(char *key) {
	for (int i = 0; i < env_var_cnt; i++) {
		if (strcmp(key, env_key[i]) == 0) {
			if (is_read_only[i]) {
				return -E_BAD_ENV_VAR;
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_set_shell_id] = sys_set_shell_id,
	...
	[SYS_print_env_var] = sys_print_env_var,
};
