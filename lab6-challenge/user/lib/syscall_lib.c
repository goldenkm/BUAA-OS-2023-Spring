...
void syscall_change_dir(char *path) {
	msyscall(SYS_change_dir, path);
}

void syscall_get_cur_path(char *path) {
	return msyscall(SYS_get_cur_path, path);
}

void syscall_set_shell_id() {
	msyscall(SYS_set_shell_id);
}

u_int syscall_get_shell_id() {
	return msyscall(SYS_get_shell_id);
}

int syscall_get_env_var(char *name, char *value) {
	return msyscall(SYS_get_env_var, name, value);
}

int syscall_set_env_var(char *key, char *value, int global, int read_only) {
	return msyscall(SYS_set_env_var, key, value, global, read_only);
}

int syscall_unset_env_var(char *key) {
	return msyscall(SYS_unset_env_var, key);
}

void syscall_print_env_var(void) {
	msyscall(SYS_print_env_var);
}